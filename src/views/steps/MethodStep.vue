<template>
  <div class="space-y-5">
    <div>
      <h2 class="text-2xl font-black" dir="auto">{{ store.t.methodTitle }}</h2>
      <p class="mt-2 text-sm leading-6 text-[#6B6756]" dir="auto">{{ store.t.methodSubtitle }}</p>
    </div>
    <div v-if="store.settingsLoading" class="flex min-h-28 items-center gap-3 rounded-xl border border-[#EFE6B8] bg-[#FFF8D7] p-4 text-sm font-bold text-[#202020]">
      ...
      <span dir="auto">{{ store.lang === 'ar' ? 'جاري تحميل طرق الدفع...' : 'Loading payment methods...' }}</span>
    </div>
    <div v-else-if="store.settingsError" class="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-800">
      <div class="flex items-start gap-3">
        ...
        <p dir="auto">{{ store.lang === 'ar' ? 'تعذر تحميل طرق الدفع المتاحة.' : 'Could not load available payment methods.' }}</p>
      </div>
      <button
      ...
      >
        ...
        <span dir="auto">{{ store.lang === 'ar' ? 'إعادة المحاولة' : 'Retry' }}</span>
      </button>
    </div>
    <div v-else-if="!store.methods.length" class="rounded-xl border border-[#EFE6B8] bg-[#FFF8D7] p-4 text-sm font-bold text-[#202020]">
      <span dir="auto">{{ store.lang === 'ar' ? 'لا توجد طرق دفع متاحة حاليا.' : 'No payment methods are available right now.' }}</span>
    </div>
    <div v-else class="grid gap-3">
      ...
      >
        ...
        <span class="min-w-0 flex-1">
          <span class="block font-black" dir="auto">{{ method.title }}</span>
        </span>
