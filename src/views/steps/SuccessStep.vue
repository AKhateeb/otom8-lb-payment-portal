<template>
  <div class="grid min-h-96 place-items-center text-center">
    <div class="max-w-md">
      ...
      <h2 class="text-3xl font-black" dir="auto">{{ store.t.successTitle }}</h2>
      <p class="mt-3 leading-7 text-[#6B6756]" dir="auto">{{ store.t.successBody }}</p>
      <section v-if="store.giftOffer" class="gift-claim-card relative mt-5 overflow-hidden rounded-xl border-2 border-[#202020] bg-[#FFF8D7] p-4 text-start shadow-lg shadow-yellow-950/10">
        ...
        <div class="relative flex items-start gap-3">
          ...
          <div class="min-w-0">
            <p class="text-xs font-black uppercase tracking-wide text-[#766600]" dir="auto">{{ store.t.giftIncluded }}</p>
            <h3 class="mt-1 text-xl font-black text-[#202020]" dir="auto">{{ store.t.giftSuccessTitle(store.giftOffer.months) }}</h3>
          </div>
        </div>

        <div v-if="store.giftApplied" class="relative mt-4 rounded-lg border border-[#31A13B] bg-[#EFFAF0] p-3 text-[#202020]">
          <p class="font-black" dir="auto">{{ store.t.giftAppliedTitle }}</p>
        ...
        <form v-else class="relative mt-4 space-y-3" @submit.prevent="store.applyGiftRenewal()">
          <div class="gift-field-focus rounded-xl border border-[#E6D470] bg-white/90 p-3 shadow-sm">
            <p class="mb-3 inline-flex items-center gap-2 rounded-full bg-[#202020] px-3 py-1 text-xs font-black uppercase tracking-wide text-[#FACE0B]">
              ...
              <span dir="auto">{{ store.t.giftClaimLabel }}</span>
            </p>
            <PhoneInput
          ...
          <p v-if="store.giftError" class="rounded-lg border border-red-200 bg-red-50 p-3 text-sm font-semibold text-red-800" dir="auto">
            {{ store.giftError }}
          </p>
          <AppButton
            type="submit"
      ...
      <div v-if="!isMobileDevice" class="mt-5 rounded-xl border border-[#EFE6B8] bg-[#FFF8D7] p-4">
        <p class="font-black text-[#202020]" dir="auto">{{ store.t.openAppOnPhoneTitle }}</p>
        <p class="mt-1 text-sm leading-6 text-[#6B6756]" dir="auto">{{ store.t.openAppOnPhoneBody }}</p>
      </div>
